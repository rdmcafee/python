# -*- coding: utf-8 -*-
"""
Created on Mon Sep 27 11:44:33 2021
...
class time_evolution:
    ...
    def gaussan_wave_packet(self,x,x0,l,a):
        ...
        self.psi_x0 = A*(np.exp((-(x - x0)**2)
            /(4*a**2))*np.exp(1j*l*x-a*x**2)).reshape(len(x),1)
    ...
    def C_n(self):
        ...
        self.Cn = self.Cn.reshape(1,quantum_number)
        ...
        for j in range(0, total_time, dt):
            ...
            for k in range(0, quantum_number):
                ...
                self.psi_xt[:,0] = self.psi_xt[:,0] + (self.Cn[0,k]
                    * self.phi[:,k] * (np.exp((-1j * self.En[0,k] * 0.05*time)/hbar)))
...
# plot
           ...
           # plt.plot(x, np.real(self.psi_xt),'r',
            ...
            plt.plot(x, self.psi_xt*np.conj(self.psi_xt),'y',
            ...
            left_wall_line = plt.axvline(-5, c='k', linewidth=2)
            right_well_line = plt.axvline(5, c='k', linewidth=2)
...
quantum_number = 500
x = np.linspace(-5,5,1000).astype(complex).reshape(1000,1)
